...
class FlattenHead(nn.Module):
    def __init__(self, n_vars, nf, target_window, head_dropout=0):
        ...
        # self.layernorm = nn.LayerNorm(nf)
...
class DualCrossAttention(nn.Module):
    def __init__(self, in_dim1, in_dim2, k_dim, v_dim, num_heads, out_dim, dropout=0.1):
        ...
        # self.linear1 = nn.Linear(in_dim1, in_dim1)

        self.cross_attention2 = CrossAttention(in_dim1, in_dim1, k_dim, v_dim, num_heads, dropout)

        # self.project_residual = nn.Linear(in_dim1, out_dim)
    ...
    def forward(self, cords_features, spatial_features, speed_features, mask=None):
        ...
        # Create weight matrix for the first cross attention
        weight_matrix1 = torch.ones(batch_size, self.cross_attention1.num_heads, nvars_patch_num, sl + sl).to(
            cords_features.device)
        weight_matrix1[:, :, :2 * patch_num, :sl] *= w  # Increase weights for first part
        weight_matrix1[:, :, 2 * patch_num:, sl:] *= w  # Increase weights for second part
        ...
        x = self.ln1(x + residual1)  # Residual Connection

        # Create weight matrix for the second cross attention
        ...
        weight_matrix2[:, :, :patch_num, :patch_num_v] *= w  # Increase weights for first part
        weight_matrix2[:, :, patch_num:2 * patch_num,
        patch_num_v:2 * patch_num_v] *= w  # Increase weights for second part
        weight_matrix2[:, :, 2 * patch_num:, 2 * patch_num_v:] *= w  # Increase weights for third part
        ...
        x = self.ln2(x + residual2)  # Residual Connection

        # Feed Forward Network with LayerNorm and Residual Connection
        residual3 = x
        x = self.ffn(x)
        x = self.ln3(x + residual3)  # Residual Connection
...
class StackedDualCrossAttention(nn.Module):
    def __init__(self, num_layers, in_dim1, in_dim2, k_dim, v_dim, num_heads, out_dim, dropout=0.1):
        ...
        # self.final_norm = nn.LayerNorm(in_dim1)
        #
        # self.final_linear = nn.Linear(in_dim1, out_dim)
...
class SWIFTModel(nn.Module):
    ...
    def forward(self, src_cords, src_sp_lon_lat, src_sp_alt, src_v, masks, lon_lat_node_features,
        ...
        src_gat = torch.cat((src_lon_lat, src_alt), dim=1)  # [bs,48,d_m]

        cords = self.dynamicInfoLayer(src_cords)  # [bs,12,d_m]

        velocity = self.dynamicInfoLayer(src_v)  # [bs,36,d_m]
